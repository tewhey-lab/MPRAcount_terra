...
print STDERR $numrep."\n";
my $out = $ARGV[0];
my @sample_ids = @ARGV[($numrep+2) .. (2*$numrep+1)];
print STDERR join("\t",@sample_ids)."\n";
my @sample_file = @ARGV[2 .. ($numrep+1)];

#open (MATCH, "$match_file") or die("ERROR: can not read file ($match_file): $!\n");

### Add in argument for the length of the array being passed
### Instead of loading into a  list do 2:n+1 are files n+2:2n+1 are IDs
...
foreach $inc (0 .. ($numrep-1))
	{
		$sample_ID = $sample_ids[$inc];
		push(@ordered_list,$sample_ID);
		$cur_file = $sample_file[$inc];
